...
const roleGrad = {
  ...
  user: "from-violet-400 to-purple-600",
};
const roleBadge = {
  admin: "bg-sky-400/10 text-sky-400 border-sky-400/20",
  manager: "bg-amber-400/10 text-amber-400 border-amber-400/20",
  user: "bg-violet-400/10 text-violet-400 border-violet-400/20",
};
const initials = (name) =>
  name
    ?.split(" ")
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display font-black text-3xl text-slate-100 tracking-tight mb-1">
            User Management
          </h1>
              ...
              <thead>
                <tr className="border-b border-white/[0.04]">
                  {["User", "Role", "Status", "Created", "Actions"].map((h) => (
                    <th
                      key={h}
                      className="px-6 py-3.5 text-left text-[10px] font-bold tracking-widest uppercase text-slate-600 whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-xl bg-gradient-to-br ${roleGrad[u.role] || roleGrad.user} flex items-center justify-center text-[12px] font-bold flex-shrink-0`}
                            style={{ color: "#080c14" }}
                          >
                            {initials(u.name)}
                          </div>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold border capitalize ${roleBadge[u.role] || roleBadge.user}`}
                        >
                          {u.role}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${active ? "bg-emerald-400/10 text-emerald-400 border-emerald-400/20" : "bg-rose-400/10 text-rose-400 border-rose-400/20"}`}
                        >
                          <span
